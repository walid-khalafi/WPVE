...
using WPVE.Web.Areas.Admin.Models.ManageViewModels;
using WPVE.Web.Extentions;
...
namespace WPVE.Web.Areas.Admin.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public async Task<IActionResult> EditProfile(IndexViewModel model)
        {
            ...
            if (model.First_Name + " " + model.Last_Name != profile.Full_Name)
            {
                if (!await _profileService.ChangeUserFullNameAsync(user.Id, model.First_Name, model.Last_Name))
                {
                    throw new ApplicationException($"Unexpected error occurred setting Avatar for user with ID '{user.Id}'.");
                }
            }
            ...
            TempData["msg"] = "پروفایل با موفقیت ویرایش شد";
        ...
        public async Task<IActionResult> EnableAuthenticator(EnableAuthenticatorViewModel model)
        {
            ...
            if (user == null)
            {
                TempData["error_msg"] = "سیستم قادر به یافتن کاربر نمی باشد";
                throw new ApplicationException($"Unable to load user with ID '{_userManager.GetUserId(User)}'.");
            }
            ...
            if (!is2faTokenValid)
            {
                TempData["error_msg"] = "کد فعالسازی اشتباه می باشد";
                ModelState.AddModelError("model.Code", "Verification code is invalid.");
                return View(model);
            }

            await _userManager.SetTwoFactorEnabledAsync(user, true);
            TempData["success_msg"] = "رمز عبور دو مرحله ای با نرم افزار Authenticator فعال شد";
        ...
        private string GenerateQrCodeUri(string email, string unformattedKey)
        {
            return string.Format(
                ...
                _urlEncoder.Encode("WCMS.WebClient"),
